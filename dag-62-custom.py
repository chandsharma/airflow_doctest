...
dag = DAG(
    "dag_62", #same name as application. and there should be no underscore in application name
...
submit_spark_job = SparkKubernetesOperator(
    task_id="submit_spark_dag_62", #NOT but in blog
    executor_config={
        "env": {
    ...
    api_group="sparkoperator.k8s.io",  # Correct API group for SparkApplication
    polling_interval=30,  # Check status every 30 seconds
    timeout=300,  # Total timeout (e.g., 5 minutes)
    dag=dag,
    # do_xcom_push=True,
